...
class LeastSquares:
    def __init__(self, model, regulator, x, y):
        self.model = model
    ...
    def __call__(self, *par):
        ...
        sel = (self.y != 0)
        chi2 = np.nansum(((self.y[sel] - ym[sel])**2/self.y[sel])) + self.regulator*(np.nanmean(ym[sel]) - 1)**2
        return chi2

def regression_least_square(model, regulator, data_x, data_y, nbr_features, **dict_ini):
    chisq = LeastSquares(model, regulator, data_x, data_y)
    m = Minuit(chisq, print_level=1, forced_parameters=[f"a{i}" for i in range(0, nbr_features + 1)], **dict_ini)
    m.migrad()
    print(m.get_param_states())
    return [m.values[f"a{i}"] for i in range(0, nbr_features + 1)]
...
def make_linear_regressor(X, Y, keep_to_train, regulator=0.0):
    ...
    dict_ini = {f'a{i}': 1 if i==0 else 0 for i in range(0, nbr_features + 1)}
    dict_ini.update({f'error_a{i}' : 0.001 for i in range(0, nbr_features + 1)})
    dict_ini.update({f'limit_a{i}': (-1, 2) if i==0 else (-1,1) for i in range(0, nbr_features + 1)})
    dict_ini.update({'errordef':1}) #for leastsquare

    param = regression_least_square(model, regulator, X_train, Y_train, nbr_features, **dict_ini)
